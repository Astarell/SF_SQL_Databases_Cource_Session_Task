WITH RECURSIVE subordinate_list AS (
    SELECT 
        ...
        emp.departmentid,
        emp.roleid,
        emp.employeeid AS root_manager_id
    FROM employees AS emp
    WHERE emp.roleid = 1 -- 1 для роли менеджера
...
FROM subordinate_list AS sl	
    ...
    JOIN subordinate_counter AS sc ON sc.managerid = sl.employeeid
ORDER BY sl.name;
